...
public class PictureReplyDAO {
	...
	public PictureDTO pictureWriteReply(PictureReplyDTO dto) {
		...
		int result = sqlSession.insert("picture_reply_write", dto);
	...
	public PictureDTO pictureWriteReReply(PictureReplyDTO dto) {
		...
		// REPLY 테이블에 추가(위와 동일)
		int result = sqlSession.insert("picture_rereply_write", dto);
	...
	public ArrayList<PictureReplyDTO> replyList(PictureReplyDTO dto) {
		...
		replyList = (ArrayList) sqlSession.selectList("picture_replyList", dto);
	...
	public PictureDTO pictureDeleteReply(PictureReplyDTO dto) {
		...
		int count_rereply = sqlSession.selectOne("picture_count_rereply", dto);
		...
		if(count_rereply==0) { //답글이 없을 때 - 그냥 삭제
			...
			// REPLY 테이블에서 삭제
			result = sqlSession.delete("picture_reply_delete", dto);
		}
		else {
			//REPLY 테이블에서 삭제
			result = sqlSession.update("pucture_reply_not_delete", dto);
		}
