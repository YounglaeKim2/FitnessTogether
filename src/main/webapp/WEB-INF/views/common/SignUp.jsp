	...
	<div id="container">
		...
		<div id="main">
			<div class="content"
				...
				<form id="dataform" action="/ft/fnt/signck.do" method="post">
					<table class="w-pct60">
						<tr>
							<th class="w-px160">닉네임</th>
							<td>
								<input type="text" id="name" name="name" onkeyup="nameKeyup()" onfocus="showTextname()" placeholder="닉네임을 입력해주세요"  value="${ loginNm }"/>
						...
						<tr>
							...
							<td>
								...
								<a class="btn default" id="idckBtn" onclick="checkid();" href="#none" 
						...
						<tr>
							...
							<td>
								<label><input type="radio" style="width: 50px; background: black; height: 20px; border: 1px;" id="gender1" name="gender" value="남"  />남자</label>
								 <span class="ico"></span> 
								<label><input type="radio" style="width: 50px; height: 20px; border: 1px;" id="gender2" name="gender" value="여" />여자</label>
							</td>
						</tr>
						<tr>
							<th>이메일</th>
							<td>								
								<input type="text" id="email" name="email" onkeyup="emailKeyup()"  placeholder="ex)kosmo12@kosmo.com"  value="${ loginEmail }"/> 
								<input type="hidden" id="emailck" name="emailck" class="chk_email" required fld_essential label="이메일중복체크" value> 
								<a class="btn default" id="emailckBtn" href="#none" onclick="checkemail();" 
								style="color: black; font-size: 15px; font-weight: bold; height: 44px; width: 100px; margin: 3.7px 8px 7px 1px; padding: 10px 10px 10px 10px; border: 2px solid #595757;">중복확인</a>
							</td>
						</tr>
						<tr>
